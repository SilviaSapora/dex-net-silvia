...
def decode_images(float_string, near_clip, far_clip, res_x=128, res_y=128):
    ...
    images = np.float32(np.concatenate([rgb, depth], axis=2))[::-1]
...
class SimulatorInterface(object):
    ...
    # @staticmethod
    # def _process_image(image, resolution):
    ...
    def create_object(self, object_path):
        ...
        r = vrep.simxCallScriptFunction(self.clientID, 'remoteApiCommandServer',
                                        vrep.sim_scripttype_childscript,
                                        'createObject', in_ints, in_floats,
                                        [object_path], bytearray(),
                                        vrep.simx_opmode_blocking)

        if r[0] != vrep.simx_return_ok:
            raise Exception('Error creating object! Return code ', r)

    def save_object(self, object_path, object_name):
        in_ints = []
        ...
        r = vrep.simxCallScriptFunction(self.clientID, 'remoteApiCommandServer',
                                        vrep.sim_scripttype_childscript,
                                        'saveObject', in_ints, in_floats,
                                        [object_path, object_name], bytearray(),
                                        vrep.simx_opmode_blocking)

        if r[0] != vrep.simx_return_ok:
            raise Exception('Error saving object! Return code ', r)
    ...
    def set_camera_pose(self, frame_work2palm, reset_config=True):
        """Sets the pose for the current object to be WRT the workspace frame.
    ...
    def run_threaded_drop(self, frame_work2obj):
        ...
        self.set_gripper_properties(visible=True, dynamic=True, collidable=True)
