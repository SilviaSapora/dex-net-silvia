...
if __name__ == '__main__':
    ...
    for ind in range(300,num_image_tensors):
        print("File: " + str(ind))
        ...
        for i in range(datapoints_in_file):
            ...
            quality_image = np.full([200,200,1], -1)
            grasp_angle_image = np.full([200,200,1], 0)
            ...
            #     for grasp_angle in range(-90,85,5):
            #         #depth_im_tf_table = depth_im.transform(translation, grasp_angle)
            #         #depth_im_tf_table = depth_im_tf_table.crop(im_crop_height, im_crop_width)
            #         #depth_im_tf_table = depth_im_tf_table.resize((im_final_height, im_final_width))
            #         grasps.append(Grasp2D(grasp_center_pt, math.radians(grasp_angle), depth_im.raw_data[x,y,0]+0.01, camera_intr=camera_intr))
